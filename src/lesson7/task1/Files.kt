...
fun centerFile(inputName: String, outputName: String) {
    ...
    PrintStream(outputName).use { printStream ->
        var length = 0
        val a = File(inputName).readLines().map {
            it.trim().let { str ->
                length = max(str.length, length)
                str
            }
        }
        a.forEach() { printStream.println(" ".repeat((length - it.length) / 2) + it) }
    }
...
fun alignFileByWidth(inputName: String, outputName: String) {
    PrintStream(outputName).use { printStream ->
        ...
        val str = File(inputName).readLines().map {
            if (it.trim().length > length) length = it.trim().length
            it.trim()
        }
        str.forEach {
            ...
            if (words.size < 2) {
                ...
                val spaceSum = length - it.length + it.count() { h -> h == ' ' }
                val space = spaceSum / (words.size - 1)
                val stringBuilder = StringBuilder()
                var ost = spaceSum % (words.size - 1)
                words.forEach { word ->
                    stringBuilder.append(word).append(" ".repeat(space + if (ost-- > 0) 1 else 0))
                }
                printStream.println(stringBuilder.toString().trim())
            }
        }
                ...
                if (letter.lowercaseChar() in dictionary) {
                    if (letter.isUpperCase()) {
                        for (i in dictionary[letter.lowercaseChar()]!!) {
                            if (counter == 0) {
                                stringBuilder.append(i.uppercaseChar())
                                counter++
                            } else stringBuilder.append(i.lowercaseChar())
                        }
